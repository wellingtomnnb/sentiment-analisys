...
class VaderConstants:
    ...
    BOOSTER_DICT = {
        ...
        "mal": B_DECR,
        "dificilmente": B_DECR,
        "apenas o suficiente": B_DECR,
        "tipo": B_DECR,
        "menos": B_DECR,
        "exeto": B_DECR,
        "exceto": B_DECR,
        "pequeno": B_DECR,
        "marginalmente": B_DECR,
...
class SentiText:
    ...
    def __init__(self, text, punc_list, regex_remove_punctuation, stopwords_file):
        ...
        # acrescenta mais algumas palavras à lista
    ...
    def _words_plus_punc(self):
        ...
        # remove stopwords - palavras indesejadas
        words_only = {w for w in words_only if w.lower() not in self.stopwords}
    ...
    def _words_and_emoticons(self):
        ...
        wes = [we for we in wes if we.lower() not in self.stopwords]
...
class SentimentIntensityAnalyzer:
    ...
    def __init__(
        self,
        lexicon_file="dados/custom_vader_lexicon.txt",
        stopwords_file="./dados/custom_stopwords.txt"
    ):
        self.stopwords_file = stopwords_file
    ...
    def polarity_scores(self, text, prin_not_lex=True):
        """
        ...
        sentitext.words_and_emoticons = self.adversatives_words_check(sentitext.words_and_emoticons)
        
        if len(sentitext.words_and_emoticons) == 0: return [{"compound":0}]
        else: 
            # Verifica se mais de 15% das palavras estão no lexico
            # caso contrário, nem prossegue
            wrds = [s.lower() for s in sentitext.words_and_emoticons]
            words_in_lex = list(filter(wrds.__contains__, self.make_lex_dict()))
            percent_words_in_lex = len(words_in_lex) * 100 / len(sentitext.words_and_emoticons)

            if prin_not_lex: print("words: %s | percent_words_in_lex: %s%%" %(len(sentitext.words_and_emoticons), percent_words_in_lex))
            if len(sentitext.words_and_emoticons) > 10 and percent_words_in_lex < 10: return [{"compound":None}]
    ...
    def _but_check(self, words_and_emoticons, sentiments):
        """
        ### Check Adversatives words in sentence
        Verifica se existe alguma palavra adversátiva e caso exista
        diminui a polaridade de toda sentença anterior a ela
        @updater: Wellington Braga
        @date: 2022-09
        """
        ...
        for word in words_and_emoticons: 
            if word in self.constants.ADVERSTIVES:
                advs_index = words_and_emoticons.index(word)

        if advs_index is not None:
            bi = advs_index
        ...
        if start_i == 0:
            if self.constants.negated([words_and_emoticons[i - 1]]):
                valence = valence * self.constants.N_SCALAR
                    ...
                    (len(words_and_emoticons) > 1) # mais de uma palavra
                    and (words_and_emoticons[i] == words_and_emoticons[-2]) # é o penultimo
                    and (
                        len(
                            list(
                                filter(words_and_emoticons[-5:-2].__contains__, self.constants.NEGATE)
                            )
                        ) == 0) # não existe nenhuma negação a 3 indexes atras
                    and (words_and_emoticons[-1] not in self.lexicon) # o ultimo não está no lexico
                ): 
                    if self.constants.negated([words_and_emoticons[-1]]): # o ultimo é uma negação
                        valence = valence * self.constants.N_SCALAR
        elif start_i == 1:

            if words_and_emoticons[i - 2] == "nunca" and (
                words_and_emoticons[i - 1] == "entao"
                or words_and_emoticons[i - 1] == "isso"
            ):
                valence = valence * 1.5
            ...
            if (
                words_and_emoticons[i - 3] == "nunca"
                and (
                    words_and_emoticons[i - 2] == "entao"
                    or words_and_emoticons[i - 2] == "isso"
                )
                or (
                    words_and_emoticons[i - 1] == "entao"
                    or words_and_emoticons[i - 1] == "isso"
    ...
    def score_valence(self, sentiments, text):
        ...
        return [sentiment_dict,sentiments]

    def adversatives_words_check(self, words:list):
        """
        ### Check Adversatives words in sentence
        Verifica se existe alguma palavra adversátiva e caso exista
        ignora toda sentença anterior a ela
        @author: Wellington Braga
        @date: 2022-09
        """
        # well custom
        # check adversatives words 
        advs_index = None
        for word in words: 
            if word in self.constants.ADVERSTIVES:
                advs_index = words.index(word)
        if advs_index is not None: words = words[advs_index:]
